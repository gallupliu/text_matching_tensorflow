...
class QACNN():
    ...
    def add_model(self, h_q, h_ap, h_am):
        ...
        real_pool_q = tf.reshape(tf.concat(axis=3, values=pool_q), [-1, total_channels])
        real_pool_ap = tf.reshape(tf.concat(axis=3, values=pool_ap), [-1, total_channels])
        real_pool_am = tf.reshape(tf.concat(axis=3, values=pool_am), [-1, total_channels])
