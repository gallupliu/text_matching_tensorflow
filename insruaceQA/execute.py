...
tf.flags.DEFINE_integer("batch_size", 32, "batch size of each batch")
tf.flags.DEFINE_integer("epoches", 150, "epoches")
tf.flags.DEFINE_integer("rnn_size", 201, "embedding size")
...
with tf.Graph().as_default():
    with tf.device("/cpu:0"):
        ...
        with tf.Session(config=session_conf).as_default() as sess:
            lstm = LSTM_QA(FLAGS.batch_size, FLAGS.quest_len, FLAGS.answer_len, embedding, FLAGS.embedding_size, FLAGS.rnn_size, FLAGS.num_rnn_layers, FLAGS.max_grad_norm)
            ...
            for epoch in range(FLAGS.epoches):
                ...
                for ori_train, cand_train, neg_train in batch_iter(ori_quests, cand_quests, FLAGS.batch_size, epoches=1):
                    run_step(sess, ori_train, cand_train, neg_train, lstm, FLAGS.dropout)
