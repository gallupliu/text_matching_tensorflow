	...
	def __init__(self, embedding_mat, non_static, hidden_unit, sequence_length, max_pool_size,
		...
		for i, filter_size in enumerate(filter_sizes):
			with tf.name_scope('conv-maxpool-%s' % filter_size):
				...
				pad_prio = tf.concat(axis=1,values=[self.pad] * num_prio)
				pad_post = tf.concat(axis=1,values=[self.pad] * num_post)
				emb_pad = tf.concat(axis=1,values=[pad_prio, emb, pad_post])
		...
		pooled_concat = tf.concat(axis=2,values=pooled_concat)
