...
class DataManager():
    ...
    def csv_to_pickle(self,infile,outfile):
        cnames=['question1', 'question2','is_duplicate']
        with open(outfile,'wb') as picklefile:
            data = pd.read_csv(infile, header=3, names=cnames, encoding='utf-8', usecols=[3, 4, 5])
    ...
    def get_embedd_matrix(self,pretrained_embeddings_dict=True):
        if pretrained_embeddings_dict:
            vector_size = 100
            glove_dir = '/home/hume/study/dl/tensorflow/kaggle-quora-solution-8th/model_hhy/deep_model/data/external/glove.6B.{0}d.txt'.format(vector_size)
            print(glove_dir)
