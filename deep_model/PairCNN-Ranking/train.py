...
def build_input_data(data_left, data_right, label, vocab):
    ...
    #print("build left:%s"%len(out_left))
    #print("build right:%s"%len(out_right))
    #print("build y:%s"%len(out_y))
    return [out_left, out_right, out_y]

def load_data(filepath, vocab_tuple=None):
    data = list(set(open(filepath,encoding='utf8').readlines()))
    ...
    data_label = []
    for x in data:
        data_left.append(x[0].strip().split(' '))
  
        data_right.append(x[1].strip().split(' '))
        data_label.append(int(x[2].strip('\n')))
    print("left:%s"%len(data_left))
    print("right:%s"%len(data_right))
    print("labels:%s"%len(data_label))
    #print(data_label)
